...
module.exports = function (thorin, storeObj, opt) {
  ...
  function getExpiredDate(opt = {}) {
    const sec = opt.expire || (expireSec * 1000);
    let expiredCreatedAt = Date.now() + sec;
  ...
  async function verifyExpiration(initialStart) {
    ...
    try {
      ...
      if (e.parent && e.parent.code === 'ER_NO_SUCH_TABLE') return;
   ...
   * */
  store.save = async function Save(id, data, done, sessionObj, opt = {}) {
    ...
    sessObj.set('expire_at', getExpiredDate(opt));
    if (customFields) {
